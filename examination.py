...
class ConversationalChat:
    ...
    async def initialize_conversation(self, language: str, level: str) -> str:
        ...
        try:
            ...
            first_conv = await self.openai_service.call_llm_with_json_output_async(system_prompt, user_prompt, ConversationalText)
            print(f"first_conv: {first_conv}")

            # 会話履歴に追加
            self.state.conversation_history.append({
                "role": "assistant", 
                "content": first_conv.message
            })
    ...
    async def continue_conversation(self, user_input: str) -> str:
        ...
        try:
            # JSON出力ではなく通常のテキスト出力に変更
            next_conv = await self.openai_service.call_llm_with_json_output_async(system_prompt, user_input, ConversationalText)

            # 会話履歴に追加
            self.state.conversation_history.append({
                "role": "assistant", 
                "content": next_conv.message
            })
            self.state.turn_count += 1
